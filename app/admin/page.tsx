...
export default async function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: "Mevcut Anahtar", value: stats.availableKeys, icon: Key, color: "bg-[#FFF785]/15 text-[#FFF785]" },
    { label: "Kullanılan Anahtar", value: stats.totalKeys - stats.availableKeys, icon: TrendingUp, color: "bg-red-100 text-[#FFF785]" },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        {cards.map((card) => (
          <div key={card.label} className="bg-[#111111] rounded-2xl border border-white/5 shadow-2xl p-5">
      ...
      <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-2xl">
        <div className="p-5 border-b border-white/5">
          <h2 className="font-bold text-white">Son Siparişler</h2>
        </div>
        {stats.recentOrders.length === 0 ? (
          <div className="text-center py-8 text-gray-500 text-sm">Henüz sipariş yok.</div>
        ) : (
            ...
            {stats.recentOrders.map((order: any) => (
              <div key={order.id} className="flex items-center justify-between px-5 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{order.game.title}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-[#FFF785]">₺{order.price.toFixed(2)}</p>
