...
const COLORS = ["#f59e0b", "#3b82f6", "#10b981", "#8b5cf6", "#FFF785"];
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={syncShopier} disabled={syncing}
            className="inline-flex items-center gap-2 border border-white/10 text-gray-200 px-4 py-2 rounded-xl text-sm font-medium hover:bg-[#0a0a0a] transition disabled:opacity-60">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: "Bu Ay Gelir", value: `₺${(stats?.monthRevenue || 0).toFixed(2)}`, icon: TrendingUp, color: "text-[#FFF785] bg-[#FFF785]/15" },
        ].map(c => (
          <div key={c.label} className="bg-[#111111] rounded-2xl border border-white/5 shadow-2xl p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        {/* Günlük Gelir */}
        <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-2xl p-5">
          <h2 className="font-bold text-white mb-4">Son 30 Gün — Günlük Gelir</h2>
        ...
        {/* Sipariş Trendi */}
        <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-2xl p-5">
          <h2 className="font-bold text-white mb-4">Son 30 Gün — Sipariş Sayısı</h2>
          <ResponsiveContainer width="100%" height={200}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Platform dağılımı */}
        <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-2xl p-5">
          <h2 className="font-bold text-white mb-4">Platform Dağılımı</h2>
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
        ...
        <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-2xl p-5">
          <h2 className="font-bold text-white mb-4">En Çok Satan Oyunlar</h2>
          <div className="space-y-3">
            {(stats?.topGames || []).map((g: any, i: number) => (
              <div key={g.title} className="flex items-center gap-3">
                <span className="w-6 h-6 rounded-full bg-[#FFF785]/15 text-[#FFF785] text-xs font-bold flex items-center justify-center">{i + 1}</span>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1.5 bg-white/5 rounded-full mt-1 overflow-hidden">
                    <div className="h-full bg-[#FFF785] rounded-full" style={{ width: `${(g.orders / (stats?.topGames?.[0]?.orders || 1)) * 100}%` }} />
                  </div>
                </div>
                <span className="text-sm font-bold text-gray-200 flex-shrink-0">{g.orders} satış</span>
              </div>
            ))}
          </div>
        </div>
